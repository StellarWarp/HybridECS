...
public:
    template <size_t I>
    using get = typename get_helper<I, T...>::type;
...
private:
    ...
    {
        using type = typename pop_back_helper<type_list<T1s..., T2>, type_list<T2s...>>::type;
    };
    ...
    struct cat_helper<type_list<T1...>>
    {
    ...
    template <typename Unsolve, typename Solved, typename BlackList>
    struct t_remove_helper;
...
private:
    ...
    template <typename... U>
    struct from_tuple_helper<std::tuple<U...>>
    {
...
public:
    ...
    using filter_with = typename filter_helper<Condition, true,
                                               type_list<type_list<>, type_list<T...>>>::type;
    template <template <typename> typename Condition>
    using filter_without = typename filter_helper<Condition, false,
                                                  type_list<type_list<>, type_list<T...>>>::type;
...
private:
    template <typename Target, int I, typename...>
    struct index_of_helper;
    template <typename Target, int I>
    struct index_of_helper<Target, I>
    {
...
private:
    template <template <typename> typename Condition, int I, typename...>
    struct find_helper;
    template <template <typename> typename Condition, int I>
    struct find_helper<Condition, I>
    {
        using type = std::integral_constant<int, -1>;
    };
