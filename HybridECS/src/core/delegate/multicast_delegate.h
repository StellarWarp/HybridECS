...
#pragma region delegate_handle
    ...
    inline delegate_handle::delegate_handle(delegate_handle_ref* obj) : delegate_object(obj) { obj->handle = this; }

    inline delegate_handle::delegate_handle(delegate_handle&& other) noexcept: delegate_object(other.delegate_object)
    {
        if (delegate_object)
        {
    ...
    inline delegate_handle::~delegate_handle()
    {
        if (delegate_object) delegate_object->handle = nullptr;
    }

    inline void delegate_handle::release()
    {
    ...
    class unique_delegate_handle_base : public delegate_handle
    {
    protected:
        friend class unique_delegate_handle_ref;

        void* container;
    public:

        using inverse_handle_type = unique_delegate_handle_ref;

        static constexpr bool container_reference = true;

        unique_delegate_handle_base() : container() {}

        unique_delegate_handle_base(void* container, unique_delegate_handle_ref* ref);

        unique_delegate_handle_base(const unique_delegate_handle_base& other) = delete;

        unique_delegate_handle_base(unique_delegate_handle_base&& other) noexcept:
    ...
    template<typename UnbindLambda>
    class unique_delegate_handle_lambda : public unique_delegate_handle_base
    {
        using super = unique_delegate_handle_base;
    public:
        unique_delegate_handle_lambda(void* container, unique_delegate_handle_ref* ref, UnbindLambda)
        : unique_delegate_handle_base(container,ref) {}
        unique_delegate_handle_lambda(const unique_delegate_handle_lambda&) = delete;
        unique_delegate_handle_lambda(unique_delegate_handle_lambda&&) = default;

        void unbind(){this->~unique_delegate_handle_lambda();}

        ~unique_delegate_handle_lambda() { if (super::container) UnbindLambda()(super::container, super::delegate_object); }
    };

    template<typename Container>
    class unique_delegate_handle_container : public unique_delegate_handle_base
    {
        using super = unique_delegate_handle_base;
    public:
        unique_delegate_handle_container() : super() {}
        unique_delegate_handle_container(void* container, unique_delegate_handle_ref* ref)
                : unique_delegate_handle_base(container,ref) {}
        unique_delegate_handle_container(const unique_delegate_handle_container&) = delete;
        unique_delegate_handle_container(unique_delegate_handle_container&&) = default;

        void unbind(){this->~unique_delegate_handle_container();}

        ~unique_delegate_handle_container() { if (super::container) static_cast<Container*>(super::container)->unbind(*this); }
    };


    class unique_delegate_handle_ref : public delegate_handle_ref
    {
        unique_delegate_handle_base* get_handle() { return static_cast<unique_delegate_handle_base*>(handle); }
    ...
    public:
        ...
        void notify_container_moved(void* new_container)
        {
        ...
        static unique_delegate_handle_ref* get(unique_delegate_handle_base* h)
        {
    ...
    inline unique_delegate_handle_base::unique_delegate_handle_base(void* container, unique_delegate_handle_ref* ref)
            : container(container), delegate_handle(ref)
    {}
...
//    using delegate_handle_t = delegate_handle_traits<DelegateHandle>::delegate_handle_type;
        using delegate_handle_t = unique_delegate_handle_container<default_delegate_container>;
        using delegate_handle_t_ref = typename delegate_handle_traits<delegate_handle_t>::delegate_handle_reference;
        using inverse_handle_t = typename delegate_handle_traits<delegate_handle_t>::inverse_handle_type;
        using inverse_handle_t_ref = typename delegate_handle_traits<delegate_handle_t>::inverse_handle_reference;
    ...
    private:
        void unbind_(delegate_handle_ref* inv_handle)
        {
    ...
    template<typename DelegateContainer, typename Ret, typename... Args> requires (not std::is_rvalue_reference_v<Args> && ...)
    ...
    public:
        ...
        //bind methods
        template<auto MemFunc, typename T_ptr> requires requires { typename std::pointer_traits<T_ptr>; }
        ...
        template<typename T, mem_func_t<T> MemFunc, typename T_ptr> requires requires { typename std::pointer_traits<T_ptr>; }
        delegate_handle_t bind(const T_ptr& obj)
        {
            return objects.bind(obj, (void*) Invoker<value_of<T_ptr>, MemFunc>);
        }
        ...
        template<typename T_ptr> requires requires{ typename std::pointer_traits<T_ptr>; }
    ...
    private:
        static Ret invoke_single(const auto& ptr, void* invoker, Args... args)
        {
            ...
            else if constexpr (requires { ptr.lock().get(); })
            {
                ...
                static_assert(!std::same_as<Ret, Ret>);//ptr get method is not implemented
