...
#define DELCATE_BINARY_OPERATOR_TEST(OP, NAME) \
	...
	{ \
		...
		static auto test(T*, U*) -> decltype(std::declval<T>() OP std::declval<U>(), std::true_type()); \
...
#define DELCATE_UNARY_OPERATOR_TEST(NAME, TEST_STATEMENT) \
	...
	{ \
		template <typename T> \
		static auto test(T*) -> decltype(TEST_STATEMENT, std::true_type()); \
		template <typename> \
		static auto test(...) -> std::false_type; \
		static constexpr bool value = decltype(test<T>(nullptr))::value; \
	}; \
...
#define DELCATE_MEMBER_FUNCTION_TEST(NAME) \
	...
	{ \
		template <typename T> \
		static auto test(T*) -> decltype(std::declval<T>().NAME(), std::true_type()); \
		template <typename> \
		static auto test(...) -> std::false_type; \
		static constexpr bool value = decltype(test<T>(nullptr))::value; \
	}; \
