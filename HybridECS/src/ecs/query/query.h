...
namespace hyecs
{
    ...
    public:
        table_tag_query(
                ...
                bool is_full_set,
                bool is_direct_set ASSERTION_CODE(, const query_condition& tag_condition)
        )
                : m_archetype_storage(base_storage),
                  m_table(base_storage->get_table()),
                  m_query_type(full_set_access),
                  m_is_direct_query(is_direct_set)ASSERTION_CODE(, m_tag_condition(tag_condition))
        {
    ...
    private:
        ...
        void notify_entity_add(entity e, storage_key key)
        {
            ...
            m_on_entity_add(e, key);
        }

        void notify_entity_remove(entity e)
        {
            m_entities.erase(e);
            m_on_entity_remove(e);
        }

    public:
        void bind_on_entity_add(function<void(entity, storage_key)> on_entity_add)
        {
            m_on_entity_add += on_entity_add;
            for (const auto& [e, st]: m_entities)
            {
                on_entity_add(e, st);
            }
        }

        void bind_on_entity_remove(function<void(entity)> on_entity_remove)
        {
            m_on_entity_remove += on_entity_remove;
        }
        ...
        void notify_partial_convert()
        {
            ...
            else
            {
                ...
                for (const auto tag_storage: m_tag_storages)
                {
                    ...
                    for (auto [e, _]: storage_entity)
                    {
                        m_entities.emplace(e, storage_key{});
                    }
                }
            }
        }
    ...
    class query
    {
        //entities set is defined by archetype_storage(no tag) and table_tag_query(with tag)
        ...
        //todo weak ref
        vector<function<void(entity, storage_key)>> m_event_copy_on_entity_add;
        vector<function<void(entity)>> m_event_copy_on_entity_remove;

    public:
        void bind_entity_event(function<void(entity, storage_key)> on_add, function<void(entity)> on_remove)
        {
            m_event_copy_on_entity_add.emplace_back(on_add);
            m_event_copy_on_entity_remove.emplace_back(on_remove);
            for (const auto storage: m_archetype_storages)
            {
                storage->bind_on_entity_add(on_add);
                storage->bind_on_entity_remove(on_remove);
            }
            for (const auto query: m_tag_table_queries)
            {
                query->bind_on_entity_add(on_add);
                query->bind_on_entity_remove(on_remove);
            }
        }
    private:

        void add_archetype_storage(archetype_storage* storage)
        {
            ...
            for (auto& on_add: m_event_copy_on_entity_add)
                storage->bind_on_entity_add(on_add);
            for (auto& on_remove: m_event_copy_on_entity_remove)
                storage->bind_on_entity_remove(on_remove);
        }

        void add_tag_table_query(table_tag_query* query)
        {
    ...
    public:
        ...
        void notify_table_query_add(table_tag_query* query)
        {
            if (query->is_direct_query())
            {
                ...
                add_archetype_storage(storage);
            }
            else
            {
                add_tag_table_query(query);
            }
        }
